...
open (LOCKFILE, '>>/home/ranger/var/run/rss-newpackages.lock') or die "could not open lockfile for append: $!";
...
$PREFIX    = '/home/ranger/var/tmp/fink-rss';
$SYNC_WAIT = 60 * 60 * 1;
...
$ENV{CVS_RSH} = '/home/ranger/cvs/rangerrick/scripts/ssh.sh';
...
if ($DOSCP)
{
	if ( -d $DISTDIR )
	{
		`cd $DISTDIR; CVS_RSH=/home/ranger/cvs/rangerrick/scripts/ssh.sh cvs -d :ext:rangerrick\@fink.cvs.sourceforge.net:/cvsroot/fink up -A >$PREFIX/dists.log 2>&1`;
	}
	else
	{
		`cd $PREFIX; CVS_RSH=/home/ranger/cvs/rangerrick/scripts/ssh.sh cvs -d :ext:rangerrick\@fink.cvs.sourceforge.net:/cvsroot/fink co dists >$PREFIX/dists.log 2>&1`;
	}
	if ( -d $EXPDIR )
	{
		`cd $EXPDIR; CVS_RSH=/home/ranger/cvs/rangerrick/scripts/ssh.sh cvs -d :ext:rangerrick\@fink.cvs.sourceforge.net:/cvsroot/fink up -A >$PREFIX/experimental.log 2>&1`;
	}
	else
	{
		`cd $PREFIX; CVS_RSH=/home/ranger/cvs/rangerrick/scripts/ssh.sh cvs -d :ext:rangerrick\@fink.cvs.sourceforge.net:/cvsroot/fink co experimental >$PREFIX/experimental.log 2>&1`;
	}
}
print "done\n";
...
if ($DOSCP)
{
	...
	for my $file (@FILES)
	{
		copy($file, '/var/www/www.finkproject.org/news/rdf/') or die "unable to copy $file to news/rdf/: $!";
		chmod(0664, '/var/www/www.finkproject.org/news/rdf/' . $file) or warn "unable to change mode on $file: $!";
	}
	print "done\n";
}
...
sub get_cvs_log
{
	...
	if (
		open( CVSLOG,
			"CVS_RSH=/home/ranger/cvs/rangerrick/scripts/ssh.sh cvs -d :ext:rangerrick\@fink.cvs.sourceforge.net:/cvsroot/fink log @keys 2>$PREFIX/cvslog.log |"
		)
	  )
	{
		$count = 0;
